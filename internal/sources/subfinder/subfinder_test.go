...
func TestSubfinder_buildCommand(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "default configuration with all sources",
			source: &SubfinderSource{
				logger:     logger,
				execPath:   "subfinder",
				timeout:    60 * time.Second,
			...
			expectArgs: []string{"-d", "example.com", "-oJ", "-silent", "-nc", "-all", "-t", "10", "-timeout", "60"},
		},
		{
		...
		},
		{
			...
			expectArgs: []string{"-d", "example.com", "-oJ", "-silent", "-nc", "-all", "-t", "20", "-rl", "100", "-timeout", "60"},
		},
	}
